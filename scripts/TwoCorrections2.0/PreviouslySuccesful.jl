...
function loss(NN_params; saveat=tsteps, ricci_weight = 1.0)
    ...
    loss = waveform_loss + tikhonov_reg + ricci_components_loss_total

    return (loss, val_wave, val_tikh, val_ricci_components, val_ricci_scalar), pred_waveform_real, pred_waveform_imag, pred_soln
end
...
for i in optimization_increments
    ...
    function scalar_loss(p)
        (loss_val, _, _, _, _), _, _, _  = tmp_loss(p)
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        (loss_val, v_wave, v_tikh, v_ricci_components, v_ricci_scalar), pred_waveform_real, pred_waveform_imag, pred_soln  = tmp_loss(p)
        ...
        plt5 = plot(losses, yscale=:log10, xlabel="Iteration", ylabel="Loss", title="Loss Curve", linewidth=2, ylims = (1e-10, 1e10), legend=:topright, primary=false)
        ...
        plt6 = plot(ricci_scalar_loss, xlabel = "Ricci Scalar Loss", ylabel = "Iteration", linewidth=2)
