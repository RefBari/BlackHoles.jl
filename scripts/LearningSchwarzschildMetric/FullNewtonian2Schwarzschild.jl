...
x_ecc, y_ecc = file2trajectory(tsteps,"input/trajectoryA_Schwarzschild_p30_e0p5.txt")
x2_ecc, y2_ecc = file2trajectory(tsteps,"input/trajectoryB_Schwarzschild_p30_e0p5.txt")
waveform_real_ecc = file2waveform(tsteps,"input/waveform_real_Schwarzschild_p30_e0p5.txt")
waveform_imag_ecc = file2waveform(tsteps,"input/waveform_imag_Schwarzschild_p30_e0p5.txt")
...
rvals_init = range(r_min, r_max; length=400)
...
function loss(NN_params; saveat=tsteps)
    ...
    loss = data_loss_real + data_loss_imag  + tikhonov + metric_asymptote 
...
for i in optimization_increments
    ...
    if i < 10
        print("i:", i, ", lr: ADAM(0.01)\n")

        local res = Optimization.solve(
            optprob, 
    ...
    elseif i == optimization_increments[end-3]  # 97
        print("i:", i, ", lr: BFGS(lr = 2)\n")

        local res = Optimization.solve(
            optprob, 
            Optim.BFGS(initial_stepnorm = 2), 
            maxiters = 4,
            callback = opt_callback
        )
    ...
    elseif i == optimization_increments[end-2]  # 98
        print("i:", i, ", lr: BFGS(lr = 1)\n")

        local res = Optimization.solve(
            optprob, 
            Optim.BFGS(initial_stepnorm = 1), 
            maxiters = 3,
            callback = opt_callback
        )
    ...
    elseif i == optimization_increments[end-1]  # 99
        print("i:", i, ", lr: BFGS = (1e-1)\n")

        local res = Optimization.solve(
            optprob, 
            Optim.BFGS(initial_stepnorm = 1e-1), 
            maxiters = 2,
            callback = opt_callback
        )
        ...
        print("i:", i, ", lr: BFGS(1e-2)\n")

        local res = Optimization.solve(
            optprob, 
            Optim.BFGS(initial_stepnorm = 1e-2), 
            maxiters = 4,
            callback = opt_callback
        )
...
# Define an r range (just outside the Schwarzschild radius)
rvals = range(6, r_max; length=400)
...
# plot!(plt1, rvals, g_tt_orig, lw=2, color=:red, label="Newtonian", xlabel=L"r", ylabel=L"g^{tt}", title=L"g^{tt}(r)")
# plot!(plt1, rvals, g_tt_pred, lw=2, color=:red, label="Predicted", xlabel=L"r", ylabel=L"g^{tt}", title=L"g^{tt}(r)")
# display(plt1)
