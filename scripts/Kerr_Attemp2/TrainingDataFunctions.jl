...
function generate_training_data(rp, ra, θmin, BH_spin, mass_ratio, θ_input, ϕ_input, max_time, num_datapoints, dt_solver, type, noise = 0.0)
    ...
    t, h₊, hₓ, Q, Qdd = nk_quadrupole_waveform(soln; a = BH_spin, μ = 1.0, θ_obs = θ_input, ϕ_obs = ϕ_input, D = 1.0)
    ...
    x1 = @. soln[2, :] * sin(soln[3, :]) * cos(soln[4, :])
    y1 = @. soln[2, :] * sin(soln[3, :]) * sin(soln[4, :])

    x2 = zeros(length(t_out))
    y2 = zeros(length(t_out))

    write_solution_txt("./input/solution_Kerr_rp"*string(rp)*"_ra"*string(ra)*"noise"*string(noise)*"_massRatio"*string(mass_ratio)*"spin_"*string(BH_spin)*"inclination_"*string(θmin)*"_time"*string(max_time)*"_observertheta_"*string(θ_input)*".txt", t_out, soln)
    write_trajectory_txt("./input/trajectoryA_Kerr_rp"*string(rp)*"_ra"*string(ra)*"noise"*string(noise)*"_massRatio"*string(mass_ratio)*"spin_"*string(BH_spin)*"inclination_"*string(θmin)*"_time"*string(max_time)*"_observertheta_"*string(θ_input)*".txt", t_out, x1, y1)
    write_trajectory_txt("./input/trajectoryB_Kerr_rp"*string(rp)*"_ra"*string(ra)*"noise"*string(noise)*"_massRatio"*string(mass_ratio)*"spin_"*string(BH_spin)*"inclination_"*string(θmin)*"_time"*string(max_time)*"_observertheta_"*string(θ_input)*".txt", t_out, x2, y2)
    write_waveform_txt("./input/waveform_real_Kerr_rp"*string(rp)*"_ra"*string(ra)*"noise"*string(noise)*"_massRatio"*string(mass_ratio)*"spin_"*string(BH_spin)*"inclination_"*string(θmin)*"_time"*string(max_time)*"_observertheta_"*string(θ_input)*".txt", t_out, h₊)
    write_waveform_txt("./input/waveform_imag_Kerr_rp"*string(rp)*"_ra"*string(ra)*"noise"*string(noise)*"_massRatio"*string(mass_ratio)*"spin_"*string(BH_spin)*"inclination_"*string(θmin)*"_time"*string(max_time)*"_observertheta_"*string(θ_input)*".txt", t_out, hₓ)
end