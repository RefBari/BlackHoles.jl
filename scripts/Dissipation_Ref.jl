...
tspan = (5.20913f3, 6.6f3) # why this time span, specifically?
datasize = 1000
...
@showprogress for i in optimization_increments
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        # plot!(plt2, x2[1:N], y2[1:N],
        # linewidth = 2, alpha = 1, color = "orange",
        # label = "orbit data 2", aspect_ratio=:equal)
        
        # plot!(plt2, pred_orbit1_nn[1,1:N], pred_orbit1_nn[2,1:N],
    ...
    if i < 80
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-2, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 50, allow_f_increases=true)
    elseif i == 80
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5e-3, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 100, allow_f_increases=true)
