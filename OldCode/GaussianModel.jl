function pe_2_EL_newtonian(semilatusRectum, eccentrcity)
    p = semilatusRectum
    e = eccentrcity
...
function GENERIC_Schwarzschild(du, u, model_params, t;
    ...
    function H(state_vec::AbstractVector)
        ...
        # nn_outputs = NN_adapter_dual(state_vec, params)
        # conservative_corrections = nn_outputs.conservative

        # f_tt_NN_correction = exp(conservative_corrections[1])
        f_rr_NN_correction = (conservative_corrections[1])
        f_ϕϕ_NN_correction = (conservative_corrections[2])
    ...
    # grad_H = ForwardDiff.gradient(x -> H(x, NN_params), x)
...
function GENERIC_Newtonian(du, u, model_params, t;
    NN=nothing, NN_params=nothing)
    #=
    Defines system of odes which describes motion of
    point like particle with Newtonian physics, uses
    ...
    # g = [
    #     -f^(-1) 0 0 0;
    #     0 ((1 + 2/r)^(-1)) * (f_rr_NN_correction) 0 0;
    #     0 0 0 0;
    #     0 0 0 r^(-2) * (1 + 2/r)^(-1) * (f_ϕϕ_NN_correction)
    # ]

    # 1. Radial Correction
    # Base: 1.0 (Flat Space). Target: 1 - 2/r.
    # r=13.3 -> Target 0.85.
    # Start: Scale 0.2, Bias 1.1 -> 1.0 - 0.2(0.75) = 0.85.
    scale_rr = 0.20
    f_rr_NN_correction = 1.0 - scale_rr * sigmoid(conservative_corrections[1] + 1.1)

    # 2. Angular Correction
    # Base: 1/r^2 (Flat Space). Target: 1/r^2.
    # Target Correction: 1.0.
    # Start: Scale 0.1, Bias -6.0 -> 1.0 + 0.1(0) = 1.0.
    scale_phi = 0.10
    # f_ϕϕ_NN_correction = 1.0 + scale_phi * sigmoid(conservative_corrections[2] - 6.0)

    # Base Metric: Time Curved, Space Flat
    # g = [
    #     -((1 - 2/r)^(-1)) 0 0 0;
    #     0 1.0 * f_rr_NN_correction 0 0;        # Base is 1.0
    #     0 0 0 0;
    #     0 0 0 r^(-2) * f_ϕϕ_NN_correction      # Base is r^-2
    # ]

    g = [
        -1 / f_rr_NN_correction 0 0 0;
        0 1 * f_rr_NN_correction 0 0;        # Base is 1.0
        0 0 0 0;
        0 0 0 r^(-2)      # Base is r^-2
    ]
