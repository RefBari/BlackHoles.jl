...
x_ecc_p14, y_ecc_p14 = file2trajectory(tsteps,"input/trajectoryA_Schwarzschild_p12_e0p4.txt")
x2_ecc_p14, y2_ecc_p14 = file2trajectory(tsteps,"input/trajectoryB_Schwarzschild_p12_e0p4.txt")
waveform_real_ecc_p14 = file2waveform(tsteps,"input/waveform_real_Schwarzschild_p12_e0p4.txt")
waveform_imag_ecc_p14 = file2waveform(tsteps,"input/waveform_imag_Schwarzschild_p12_e0p4.txt")
...
for (i, layer) in enumerate(NN_Conservative_params)
    if ~isempty(layer)
        if i == length(NN_Conservative_params)  # Final layer
            layer.weight .= 0
            layer.bias .= -10 # Force output near 0
        else  # Hidden layers
            layer.weight .= 0
            layer.bias .= 0
        end
...
plot(x_ecc_p14[1:20], y_ecc_p14[1:20], aspect_ratio=:equal, linewidth = 2, label = "Real Orbit")
plot!(pred_orbit1_nn_p14[1,1:16], pred_orbit1_nn_p14[2,1:16], aspect_ratio=:equal, linewidth = 2, label = "Optimized Prediction")
