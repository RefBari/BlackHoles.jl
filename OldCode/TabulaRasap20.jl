...
x_ecc, y_ecc = file2trajectory(tsteps,"input/trajectoryA_Schwarzschild_p20_e0p1.txt")
x2_ecc, y2_ecc = file2trajectory(tsteps,"input/trajectoryB_Schwarzschild_p20_e0p1.txt")
waveform_real_ecc = file2waveform(tsteps,"input/waveform_real_Schwarzschild_p20_e0p1.txt")
waveform_imag_ecc = file2waveform(tsteps,"input/waveform_imag_Schwarzschild_p20_e0p1.txt")
...
function grr_pred(r, NN_params)
    ...
    f_rr = -sigmoid(out[1]-4)
    open("TabulaRasa_Results_p20_Newtonian.txt", "a") do io
        println(io, "\nr:", r, " \n[ Raw gᵣᵣ Correction: ", out[1], "| gᵣᵣ Correction (after -1*sigmoid(out[1]-4)):", f_rr, " ]")
...
function gϕϕ_pred(r, NN_params)
    out, _ = NN_Conservative([r], NN_params.conservative, NN_Conservative_state)
    f_ϕϕ = sigmoid(out[2] - 4)
    open("TabulaRasa_Results_p20_Newtonian.txt", "a") do io
        println(io, "\n[ Raw gϕϕ Correction: ", out[2], "| gϕϕ Correction (after sigmoid(out[2]-4)): ", f_ϕϕ, " ]\n")
        println(io, "\n[ Current gϕϕ Value: ", r^(-2) * (1+(2/r))^(-1) * (1 + f_ϕϕ), "| True gϕϕ Value: ", r^(-2), " ]\n")
    end    
    print("\n[ Raw gϕϕ Correction: ", out[2], "| gϕϕ Correction (after sigmoid(out[2]-4)): ", f_ϕϕ, " ]\n")
    print("[ Current gϕϕ Value: ", r^(-2) * (1+(2/r))^(-1) * (1 + f_ϕϕ), "| True gϕϕ Value: ", r^(-2), " ]\n")
...
plot(x_ecc[:], y_ecc[:], aspect_ratio=:equal, linewidth = 2, label = "Real Orbit")
plot!(pred_orbit1_nn[1,:], pred_orbit1_nn[2,:], aspect_ratio=:equal, linewidth = 2, label = "Optimized Prediction")
...
plot!(grr_newtonian_final, label = "Final g^rr Newtonian", linewidth = 2)
...
plot!(gϕϕ_newtonian_final, label = "Final g^ϕϕ Newtonian", linewidth = 2)
