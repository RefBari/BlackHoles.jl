...
function make_u0(params)
    E0 = E0_base + 1e-3 * sigmoid(params.dE0)
    L0 = L0_base + 1e-3 * sigmoid(params.dL0)
...
r_vals_initial_testing = range(r_soln_min, r_soln_max, length = 100)
...
initial_grr_pred_plot = plot(grr_pred_initial, title = L"g^{rr}", label = "Initial g^rr Predicted", linewidth = 2)
plot!(grr_newtonian_array, label = "Initial g^rr Newtonian", linewidth = 2)
plot!(grr_schwarzschild_array, label = "Initial g^rr Schwarzschild", linewidth = 2)  

initial_gϕϕ_pred_plot = plot(gϕϕ_pred_initial, title = L"g^{ϕϕ}", label = "Initial g^ϕϕ Predicted", linewidth = 2)
plot!(gϕϕ_newtonian_array, label = "Initial g^ϕϕ Newtonian", linewidth = 2)
plot!(gϕϕ_schwarzschild_array, label = "Initial g^ϕϕ Schwarzschild", linewidth = 2)
...
for i in optimization_increments
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        r_vals_training = range(r_vals_training_min, r_vals_training_max, length = 100)
        ...
        training_grr_pred_plot = plot(grr_pred_training, label = "Training g^rr Predicted", linewidth = 2)
        plot!(grr_newtonian_training, label = "Training g^rr Newtonian", linewidth = 2)
        plot!(grr_schwarzschild_training, label = "Training g^rr Schwarzschild", linewidth = 2)  
    ...
    if i < 10
        print("i:", i, ", lr: 10\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 50)
    elseif i == 10
        print("i:", i, ", lr: 5\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=2, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 40)
    elseif i == optimization_increments[end-3]  # 97
        print("i:", i, ", lr: 3\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 20)
    elseif i == optimization_increments[end-2]  # 98
        print("i:", i, ", lr: 2\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-1, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 10)
    elseif i == optimization_increments[end-1]  # 99
        print("i:", i, ", lr: 1\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-2, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 5)
    else  # 100'
        print("i:", i, ", lr: 1e-1\n")
...
r_vals_final_testing = range(r_soln_min_final, r_soln_max_final, length = 100)
...
for i in range(1, length(r_vals_final_testing))
    ...
    print("g^rr Corrections (Should be 0): ", grr_pred(r_vals_final_testing[i], NN_params)[2], "\n")
