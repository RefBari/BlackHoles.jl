...
for i in range(1, length(r_vals_initial_testing))
    ...
    print("g^rr Corrections (Should be 0): ", grr_pred(r_vals_initial_testing[i], NN_params)[2], "\n")
...
initial_grr_pred_plot = plot(grr_pred_initial, label = "Initial g^rr Predicted", linewidth = 2)
...
initial_gϕϕ_pred_plot = plot(gϕϕ_pred_initial, label = "Initial g^ϕϕ Predicted", linewidth = 2)
