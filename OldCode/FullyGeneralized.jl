...
# u0_pe_coordinates = Float32[0.0, 0.0, 6.484530872637143, 0.16384305990318113]  # χ₀ = 0, φ₀ = 0 (periapsis)
...
soln_simple = solve(prob_simple, saveat = tsteps, dt = dt)
...
plt = plot(tsteps_actual, waveform_real_ecc[1:n_actual],
...
NN_Dissipative = Chain(
    Dense(3, 4, tanh), # Input: r, p_r, p_\phi
    Dense(4, 4, tanh),
    Dense(4, 1),
)
...
function NN_adapter(u, params)
    features = [u[2], u[6], u[8]]  # t, r, θ, φ, p_t, p_r, p_θ, p_ϕ
...
soln_nn = Array(solve(prob_nn, saveat = tsteps, dt = dt, adaptive=false, verbose=false))
...
function loss(NN_params; saveat=tsteps)
    ...
    pred_soln = solve(remake(prob_nn, p = NN_params, tspan=tspan),
...
for i in optimization_increments
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        plt1 = plot(tsteps[1:N], waveform_real_ecc[1:N], 
                    markershape=:circle, markeralpha = 0.25,
    ...
    if i < 80
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-1, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 50, allow_f_increases=true)
    elseif i == 80
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5e-2, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 100, allow_f_increases=true)
    elseif i == optimization_increments[end-3]  # 97
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-2, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 200, allow_f_increases=true)
    elseif i == optimization_increments[end-2]  # 98
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5e-3, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 300, allow_f_increases=true)
    elseif i == optimization_increments[end-1]  # 99
        ...
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5e-4, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 500, allow_f_increases=true)
    end
...
# # plot!(plt2, x_ecc[1:maxSteps], y_ecc[1:maxSteps], linewidth = 2, alpha = 0.5, label="true eccentric inspiral", color = "blue")
