...
# E0_base_p14, L0_base_p14 = pe_2_EL(p_14, e_14)[2:3] # Schwarzschild IC
r0_p14 = p_14 / (1 - e_14)
...
function make_u0_p14(params)
    ...
    ΔL = 0.2     # L0 ∈ [L0_base - 1.5, L0_base + 1.5]
...
r_min = minimum(soln_nn_dual_p14[2,:])
r_max = maximum(soln_nn_dual_p14[2,:])
r_soln_min = r_min
r_soln_max = r_max
...
function loss(NN_params; saveat=tsteps)
    ...
    N = length(pred_waveform_real_p14)

    data_loss_real = ( sum(abs2, waveform_real_ecc_p14[1:N] .- pred_waveform_real_p14)) / (sum(abs2, waveform_real_ecc_p14[1:N]))
    data_loss_imag = ( sum(abs2, waveform_imag_ecc_p14[1:N] .- pred_waveform_imag_p14)) / (sum(abs2, waveform_imag_ecc_p14[1:N]))
    ...
    return loss, pred_waveform_real_p14, pred_waveform_imag_p14, pred_soln_p14, NN_params
end
...
for i in optimization_increments
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        plt1 = plot(tsteps[startPoint:N], waveform_real_ecc_p14[startPoint:N], 
        ...
        plt3 = plot(x_ecc_p14[startPoint:N], y_ecc_p14[startPoint:N],
    ...
    if i < 10
        print("i:", i, ", lr: 1\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=3, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 50)
    elseif i == 10
        print("i:", i, ", lr: 1e-1\n")
