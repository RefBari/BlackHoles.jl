...
function template_SNR(h::AbstractVector, dt)

    #=
        Compute (time-domain) GW template used in matched filter (Using Euclidearn inner product, no PSD)
        INPUT ===
        h: gw signal (real numpy array)
    =#

    norm = sqrt( dt * sum(abs2,h) )
    return h/norm, norm
end

function compute_SNR(template::AbstractVector, data::AbstractVector, dt)

    @assert length(data) == length(template) "input vectors must be the same length"

    return dt * sum(data .* template)
end

function set_SNR(h::AbstractVector, dt, SNR)
    h_template, norm = template_SNR(h,dt)
    noise = randn(eltype(h), size(h))
    data = noise + SNR*h_template
    # compute SNR (its a random variable; so won't be SNR value we request)
    measured_snr_1 = compute_SNR(data, h_template, dt)
    # scale amplitude to get the target SNR
    signal = (SNR^2/measured_snr_1)*h_template
    data = noise + signal
    measured_snr_2 =  compute_SNR(data, h_template, dt)
    # print("Measured 1 = ", measured_snr_1," measured 2 = ", measured_snr_2)
    # return data
    return data / SNR * norm
end
