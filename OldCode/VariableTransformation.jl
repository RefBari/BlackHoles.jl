...
for r in rvals_init
    out, _ = NN_Conservative([1/r], NN_params.conservative, NN_Conservative_state)
...
function loss(NN_params; saveat=tsteps)
    ...
    reg_loss = 1e-9 * sum(abs2, NN_params)
...
for i in optimization_increments
    ...
    if i < 10
        print("i:", i, ", lr: 5\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=10, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=true)
    elseif i == 10
        print("i:", i, ", lr: 2\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=5, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=true)
    elseif i == optimization_increments[end-3]  # 97
        print("i:", i, ", lr: 1e-1\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=3, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=true)
    elseif i == optimization_increments[end-2]  # 98
        print("i:", i, ", lr: 1e-2\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=2, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=true)
    elseif i == optimization_increments[end-1]  # 99
        print("i:", i, ", lr: 1e-3\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=true)
    else  # 100'
        print("i:", i, ", lr: 1e-4\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-1, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 30, allow_f_increases=true)
    end
