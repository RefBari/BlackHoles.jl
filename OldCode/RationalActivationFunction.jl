...
struct RationalAct <: Lux.AbstractLuxLayer
end

# Trainable parameters: numerator coeffs `a[1:4]`, denominator coeffs `b[1:3]`
function Lux.initialparameters(::AbstractRNG, ::RationalAct)
    # ReLU-like initialization from the rational NN paper
    a = Float32[1.1915, 1.5957, 0.5, 0.0218]
    b = Float32[2.3830, 0.0, 1.0]
    return (a = a, b = b)
end

# No state for this layer
Lux.initialstates(::AbstractRNG, ::RationalAct) = NamedTuple()
Lux.statelength(::RationalAct) = 0

# Forward pass: elementwise rational function, works on scalars and arrays
function (layer::RationalAct)(x, ps, st)
    a = ps.a
    b = ps.b

    # Horner scheme for numerical stability
    num = a[1] .+ x .* (a[2] .+ x .* (a[3] .+ x .* a[4]))
    den = b[1] .+ x .* (b[2] .+ x .* b[3])

    eps_val = eps(eltype(num))
    y = num ./ (den .+ eps_val)
...
NN_Rational = Chain(
    Dense(1 => 10), RationalAct(),
    Dense(10 => 10), RationalAct(),
    Dense(10 => 2)          # usually no activation on final layer
)
...
NN_Conservative_params, NN_Conservative_state = Lux.setup(rng, NN_Rational)
...
final_layer = NN_Conservative_params.layer_3
final_layer.bias[1] = -10
final_layer.bias[2] = -10
...
function NN_adapter_dual(u, params)
    ...
    conservative_output, _ = NN_Rational(conservative_features, params.conservative, NN_Conservative_state)
...
function grr_pred(r, NN_params)
    out, _ = NN_Rational([r], NN_params.conservative, NN_Conservative_state)
    f_rr = -1e-1*sigmoid(out[1])
    print("\nRaw gᵣᵣ Correction: ", out[1], "| gᵣᵣ Correction:", f_rr)
...
function gϕϕ_pred(r, NN_params)
    ...
    print("\nRaw gϕϕ Correction: ", out[2], "| gϕϕ Correction:", f_ϕϕ)
