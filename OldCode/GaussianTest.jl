...
# Baseline inverse metric pieces
function gtt_baseline(r)
    f = 1 - 2/r
    return -1/f                  # same as before
end
...
function gtt_NN(r, params)
    # if you *don't* correct g^{tt} with the NN:
    return gtt_baseline(r)
end
...
#     ΔL = 0.4     # L0 ∈ [L0_base - 1.5, L0_base + 1.5]
...
#     raw_E = params.dE0_p14
#     raw_L = params.dL0_p14
...
plot!(pred_orbit1_init[1,1:15], pred_orbit1_init[2,1:15], aspect_ratio=:equal, linewidth = 2, label = "Prediction")
...
for i in optimization_increments
    ...
    print("The final optimized parameters are:", NN_params.conservative.layer_3,
          " |\nFor (p = 12, e = 0.5) --> E0: ", E0_base + 1e-2 * sigmoid(NN_params.dE0), " | L0: ", L0_base - 0.4 * sigmoid(NN_params.dL0), ", True (E0, L0):", pe_2_EL(p, e)[2:3],
          " |\nFor (p = 14, e = 0.5) --> E0: ", E0_base_p14 + 1e-2 * sigmoid(NN_params.dE0_p14), " | L0: ", L0_base_p14 - 0.4 * sigmoid(NN_params.dL0_p14), ", True (E0, L0):", pe_2_EL(p_14, e_14)[2:3],)
end
