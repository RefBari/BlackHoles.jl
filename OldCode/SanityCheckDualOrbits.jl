...
x_ecc_p14, y_ecc_p14 = file2trajectory(tsteps,"input/trajectoryA_Schwarzschild_p20_e0p1.txt")
x2_ecc_p14, y2_ecc_p14 = file2trajectory(tsteps,"input/trajectoryB_Schwarzschild_p20_e0p1.txt")
waveform_real_ecc_p14 = file2waveform(tsteps,"input/waveform_real_Schwarzschild_p20_e0p1.txt")
waveform_imag_ecc_p14 = file2waveform(tsteps,"input/waveform_imag_Schwarzschild_p20_e0p1.txt")
...
function gphiphi_NN(r, params)
    ...
    return gphiphi_baseline(r) * (1 + f_phiphi)   # or * exp(f_phiphi) if you want
end
...
function loss(NN_params; saveat=tsteps)
    ...
    pred_soln_p14 = solve(prob_p14, Tsit5();
                      ...
                      adaptive=false, verbose = false, 
                      sensealg=BacksolveAdjoint(checkpointing=true))
    pred_waveform_real_p14, pred_waveform_imag_p14 = compute_waveform(dt_data, pred_soln_p14, mass_ratio, coorbital=false)
...
for i in optimization_increments
    ...
    function opt_callback(state, args...; kwargs...)
        ...
        plt2 = plot(tsteps[startPoint:N], waveform_real_ecc_p14[startPoint:N], 
                    markershape=:circle, markeralpha = 0.25,
                    linewidth = 2, alpha = 0.5, label = "True Gravitational Wave (p = 12, e = 0.4)")
        plot!(plt2, tsteps[startPoint:N], pred_waveform_real_p14[startPoint:N],
              markershape=:circle, markeralpha = 0.25,
              linewidth = 2, alpha = 0.5, label = "Predicted Gravitational Wave (p = 12, e = 0.4)")    

        wavePlots = plot(plt1, plt2, layout = (1,2))
        display(wavePlots)
        ...
        for i in range(1, length(r_vals_training))
            print("\nr:", r_vals_training[i])
            print("\ng^rr Corrections (Should be 0): ", grr_pred(r_vals_training[i], NN_params)[2], "\n")
        ...
        plt4 = plot(x_ecc_p14[startPoint:N], y_ecc_p14[startPoint:N], lw=2, alpha=1, label="True Orbit (p = 12, e = 0.4)", camera=(35,20))
        plot!(plt4, pred_orbit1_p14[1,startPoint:N], pred_orbit1_p14[2,startPoint:N], lw=2, alpha=0.9, ls=:dash, color=:red, label="Predicted Orbit (p = 12, e = 0.4)")

        orbitPlots = plot(plt3, plt4, layout = (1,2))
        display(orbitPlots)
    ...
    if i < 10
        print("i:", i, ", lr: 1\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 50)
    elseif i == 10
        print("i:", i, ", lr: 1e-1\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-1, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 40)
    elseif i == optimization_increments[end-3]  # 97
        print("i:", i, ", lr: 1e-2\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-2, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 20)
    elseif i == optimization_increments[end-2]  # 98
        print("i:", i, ", lr: 1e-3\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-3, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 10)
    elseif i == optimization_increments[end-1]  # 99
        print("i:", i, ", lr: 1e-4\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-4, linesearch = LineSearches.BackTracking()), callback=opt_callback, allow_f_increases=false, iterations = 5)
    else  # 100'
        print("i:", i, ", lr: 1e-5\n")
        local res = Optimization.solve(optprob, Optim.BFGS(; initial_stepnorm=1e-5, linesearch = LineSearches.BackTracking()), callback=opt_callback, maxiters = 30, allow_f_increases=true, iterations = 1)
    end
